...
export default function ParsecStatus() {
  ...
  async function fetchStatus(maxRetries = 3) {
    ...
    setLoading(false)
        ...
        console.error(`Failed to fetch status (attempt ${attempt + 1}/${maxRetries}):`, err)
  ...
  async function updateStatusShouldBe(newValue: StatusValue) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        console.log(`Update attempt ${attempt + 1}/${maxRetries}: Fetching fresh SHA...`)
        ...
        if (res.ok) {
          ...
          console.warn(`SHA conflict on attempt ${attempt + 1}:`, errorData.message)
          ...
          if (attempt < maxRetries - 1) {
            ...
            throw new Error(`SHA conflict after ${maxRetries} attempts: ${errorData.message}`)
          }
        } else {
          throw new Error(`GitHub API error: ${res.status} ${res.statusText}`)
        }
  ...
  return (
    ...
    {data ? (
    <Stack spacing={2} alignItems="center" justifyContent="center" height="100vh">
        {loading || updating ? <Box><IOSLoader size={40} /></Box> : <Box sx={{ height: '45.5px', width: '40px' }} />}
      ...
      {data.failureReason && (
        <Typography width="75%" color="error" align="center" variant="body2">
          ⚠️ Error: {data.failureReason}
        </Typography>
      )}
      
      {error && (
        <Typography width="75%" color="warning.main" align="center" variant="body2">
